...
Several new methods are suggested. Questions of convergency, computational complexity and accuracy are considered for the proposed methods. The methods are compared on a numeric example.
\end{abstract}
...
The structure of the paper is as follows.  In Section~\ref{sec:lowrank_appr}, a problem of approximation of a matrix by a Hankel rank-deficient matrix is considered. Common structure of iterative alternating projection algorithms is described, approaches to construction of the projectors are given, the convergency theorem is proved.
...
The paper is finished with conclusions (Section~\ref{sec:concl}) The result of separability of constant and sinusoidal signals by SSA with weights, which has a connection with convergency for two of the described algorithms, is proved in Appendix.
...
Convergency of outer iterations by subsequences was proved for the reviewed class of algorithms.
