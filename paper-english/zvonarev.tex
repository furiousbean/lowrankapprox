...
a weighted least-squares method is applied to the trajectory matrix of the considered time series to obtain an approximation of the original time series. The aim of matrix weights is to obtain equal or approximately equal weights in the equivalent problem of time series least-squares approximation.
Several new methods are suggested. Questions of convergence, computational complexity and accuracy are considered for the proposed methods. The methods are compared on numeric examples.
\end{abstract}
...
Weights on edges of series are less than that in the center, i.e. the ordinary least-square problem \eqref{introd_task} for matrices corresponds to a weighted least squares problem for series.

The aim of this paper is to consider methods which solve problem \eqref{introd_task_2} with equal weights instead of $w_i$ and to compare the constructed methods in terms of accuracy of the signal estimation. All described methods are iterative. 
...
Hence, the described methods are compared by accuracy of the signal estimation at the first iteration and in the limit. Note that Singular Spectrum Analysis (SSA) \cite{Broomhead.King1986, Vautard.etal1992, Elsner.Tsonis1996, Golyandina.etal2001, Ghil.etal2002, Golyandina.Zhigljavsky2012} can be
...
The paper is summarized and conclusions are drawn in Section~\ref{sec:concl}. Supplementary results on separability by SSA with weights, which has a connection with convergence rate, are proved in Appendix A.
...
\begin{proposition}
	\label{prop:projS}
	Let $\bfC$ be a symmetric semidefinite matrix of size $K \times K$ such that $\|\bfY\|_\bfM^2 = \tr(\bfY \bfC \bfY^\rmT)$.
...
hence, properties of EM-algorithms are carried out and the sequence $\bfY_k$ converges to local minimum. Formally, it does not matter what values are in $\bfY$ at positions of zero weights. However, these values can influence the algorithm's convergence rate.
...
$q_i \ge 0$, $i = 1, \ldots, N$. The one of reasonable target functions is the squared Euclidean distance in $\sfR^N$. It coincides with $f_q(\tsY)$ when $q_i = 1$, $i = 1, \ldots, N$.

\paragraph*{Adjustment.} For a numeric solution of optimization problem \eqref{L-rank_task} the following fact is important.
Consider a Hilbert space $\calX$ with inner product $\langle\cdot, \cdot\rangle$, $\calM \subset \calX$ is closed with respect to scalar multiplication. Let an element $x$ belong to $\calX$, $y$ belong to $\calM$. Consider a projection $y^*$ of an element $x$ to a line $l = \{\alpha y:\, \alpha \in \sfR\}$. Then for the adjusted element $y^*$
\begin{equation}
\label{eq:adjust}
...
In the space $\sfX_N$ of time series, the target function \eqref{L-rank_task} is given explicitly using (semi)inner product
\begin{equation}
\label{eq:norm_ser}
...
Consider two (semi)inner products in the space $\sfR^{L \times K}$ of matrices which are extension of conventional Frobenius inner product.
...
\begin{proof}
	...
	\end{equation*}
The proof of the second statement is a consequence of the fact that for only for a diagonal matrix $\bfC$ the corresponding inner product has a form appropriate to \eqref{eq:norm1M}:
	\begin{equation*}
...
\begin{corollary}
	...
	If $m_{i,j}=1$, $i =1, \ldots, L$, $j = 1, \ldots, K$, then the equivalent series weights $q_i$, $i = 1, \ldots, N$, are equal to $w_i$ given in \eqref{eq:w}.
\end{corollary}
...
\begin{remark}
	\label{rem:2tasks}
	Thus, if condition~\eqref{qi_mi} is carried out and all weights $q_i$ and $m_{i,j}$ are positive, then problem~\eqref{L-rank_task}
	is equivalent to the problem
	\begin{equation}
	\label{rank_task}
	f_\bfM(\bfY) \to \min_{\bfY \in \calM_r \cap \calH}, \quad f_\bfM(\tsY) = \|\bfX-\bfY\|_{1,\bfM}.
	\end{equation}
\end{remark}
...
In the model of series $\tsX=\tsS+\tsN$, where $\tsS$ is a time series of finite rank $r$ and $\tsN$ is a noise, the result the algorithms serves as an estimates of the signal $\tsS$.

\subsection{Cadzow iterations}
The aim of the Cadzow algorithm \cite{Cadzow1988} is approximation of the trajectory matrix of a series with respect to the norm $\|\cdot\|_{1, \bfM}$ with the weights $m_{ij}=1$ (i.e. the algorithm solves problem \eqref{introd_task}, which, by Corollary \ref{cor:base_weights}, corresponds to problem~\eqref{introd_task_2} with the weights $w_i$ given in \eqref{eq:w}). The drawback of this algorithm consists in the unequal series weights $w_i$: they are larger in the center than at both ends of the time series. Note that smaller window lengths leads to more uniform weights.
...
Let $q_{i}=1$, $i = 1, \ldots, N$, in \eqref{L-rank_task}. According to Proposition ~\ref{prop:equiv_tasks},  problem \eqref{L-rank_task} is equivalent to problem \eqref{rank_task} with weights
\begin{equation}
\label{Mw}
...
The Extended Cadzow algorithm presents different approach to problem \eqref{L-rank_task} with arbitrary weights than the Weighted Cadzow algorithm.
Formally, the series $\tsX$ is extended to both sides on $L-1$ measurements with some values having zero weights, i.e., the added measurements are considered as gaps.
...
For the extended series, Algorithm~\ref{alg:weightedSVD} with weights $m_{i,j}=\calT \tsI$ is applied to $\widetilde\bfX$, where the series $\tsI$ has ones in the place of the given series $\tsX$ and zeroes in positions of gaps, i.e.
\begin{equation*}
m_{i,j} = \begin{cases}
1 & 1 \le i+j-L \le N, \\
0 & \text{otherwise.}
...
\begin{algorithm}[Extended Cadzow iterations]\label{alg:ECIt}
	...
	\begin{enumerate}
		...
		\item
		Construct $\bfY_k$ consisting of the columns from $L$-th to $N$-th ones  of the matrix $\widetilde\bfY_{k}$. If STOP1 then $\widehat\tsS = \calT^{-1} \bfY_k$; else go to 2.
	\end{enumerate}
\end{algorithm}
...
The following lemma describe a case, when the conditions of Proposition\ref{prop:equiv_tasks} are fulfilled for equivalence of \eqref{L-rank_task} with equal weights $q_i$ and \eqref{rank_task}.
...
This approach has an important drawback. Since zeroes are placed at the diagonal of the diagonal matrix $\bfC$, $\bfC$ has rank $h$, which is considerably smaller than $K$. The change of diagonal zeroes to some small $\alpha$ is suggested in \cite{Gillard2014} to improve rank-deficiency.
...
\end{cases}
\end{multline}
Then the matrix $\bfC$ with the diagonal given in \eqref{zhigweights} is of full rank.

Let us denote Cadzow($\alpha$) the iterations performed by Algorithm~\ref{alg:obliqueCadzow} with the diagonal matrix $\bfC=\bfC(\alpha)$  given in \eqref{zhigweights}.
...
\begin{remark}
	The optimization problem \eqref{rank_task} with the matrix $\bfM$ given in \eqref{eq:degenerateM} corresponds to the search of an arbitrary (not necessary Hankel) matrix of rank not larger than $r$, which is closest in Frobenius norm to the matrix
	\be
...
To illustrate the form of weights $\hat{q_i}$, let us formulate propositions with simplifying conditions.
\begin{proposition} \label{myweightstat}
	Let $N \ge 2(L-1)$. Then the diagonal matrix weights $\hat c_k$ defined in \eqref{my_s} are equal to
	\begin{equation*}
	\hat c_k = \begin{cases}
	\frac{1}{L}\left(\frac{k}{L} + \sum_{j=k}^{L-1} \frac{1}{j} \right),& k = 1, \ldots, L-1, \\
	1/L, &k = L, \ldots K - L + 1,\\
	\hat c_{K - k + 1}, & k = K - L + 2, \ldots K.
 	          \end{cases}
	\end{equation*}
\end{proposition}
...
\begin{proof}
	...
	=\frac{i - L + 1}{L} + \frac{1}{L^2} \sum_{j = i - L + 1}^{L-1}j + \frac{1}{L} \sum_{j = i-L + 1}^{L-1} \sum_{k=j}^{L-1}\frac{1}{r} =\\
	...
	\end{multline*}
	The weights $\hat{q}_i$ for $N-2L+2 \le i \le N$ are calculated by symmetry. The center series elements are evidently equals 1.
\end{proof}

Let us normalize series weights so that their sum equals 1. 
Normalized weights $q_i(\alpha)$, for $\alpha = 1$ (conventional Cadzow iterations), $\alpha = 0$ (equal $q_i$), $\alpha = 0.1$,
...
%consisting of columns of matrices $\Pi_{\calM_r} \bfY_k$ from $L$-th to $N$-th and having positive weight of each element are matrices of rank not larger than $r$, and their Frobenius norms are not larger than seminorm of original matrix: $\|\widetilde \bfY_k\|_F \le \|\Pi_{\calM_r} \bfY_k\|_\bfM$. Hence it is bounded sequence which contains a subsequence converging to needed set.
		\item \textit{Convergence in practice.} Although the theory says about existence of converging subsequences, the convergence took place in all considered numerical examples.
		\item \textit{Global or local minimum.}
		The methods are iterative, and convergence to the global minimum in the corresponding least-squares problem does not necessarily takes place. Therefore, different algorithms corresponding to the same weights can yield different approximations. Thereby, comparison of the algorithms by the approximation precision has sense.
\item \textit{Signal estimation and series approximation.}
		The considered methods can be considered as weighted least-squares methods (that is, approximation of the initial series by finite-rank series) for signal estimation. Note that generally the approximation quality can contradict to the estimation accuracy due to possible over-fitting. 
		\item \textit{Algorithms and series weights.}
		The Weighted Cadzow and Extended Cadzow methods try to solve problem \eqref{L-rank_task} with equal weights $q_i$. Other methods work with weights with various degree of non-uniformity.
		\item \textit{Algorithms and computational costs.}
		All suggested algorithms are iterative. However, each outer iteration in the Weighted Cadzow and Extended Cadzow algorithms has a step with inner iterations. Therefore, these algorithms are very time-consuming. Other algorithms have similar computational costs of one iteration.
		Computational complexity is described by both complexity of one iteration and the number of iterations. Evidently, the number of iterations is determined by convergence rate.
		...
		Separability of the signal, which is an important concept of the SSA method, means the ability of a method to (approximately) separate the signal from a residual. From the viewpoint of the iterative methods,  separability quality is closely related to accuracy of the first iteration of the method. On the other hand, we can expect that the accuracy of the first iteration is connected with the method's convergence rate. Therefore, the separability accuracy is related to the convergence rate of iterative methods.
		\item \textit{Separability and choice of parameters.}
		The connection between separability and the window length $L$ is well researched for the SSA method, see \cite{Golyandina2010}. In particular, optimal window lengths are close to half of the series length. A small window length $L$ provides poor separability. We can expect that this is valid for the other algorithms too.
The Cadzow($\alpha$) method has an additional parameter $\alpha$.
Influence of parameter $\alpha$ in the class of Cadzow($\alpha$) iterations is investigated in Appendix~\ref{sec:app} on the example of separability of a sine-wave signal from a constant residual. It is shown that small values of $\alpha$ provide poor separability.
...
There are no theoretical results about the behavior of the estimation accuracy in dependence on algorithms and their parameters. However, the numerical study shows that the best accuracy is achieved in the algorithms corresponding to the weights, which are equal or almost equal.	 \end{itemize}
	...
	\begin{remark}
		\label{rem:adjust}
		Adjustment defined in \eqref{eq:adjust} can be applied to the resultant signal estimation $\widehat\tsS$ for any considered algorithm. In \eqref{eq:adjust} $\|\cdot\|$ is standard Euclidean norm not depending on algorithms, since this norm corresponds to problem \eqref{L-rank_task} with equal weights $q_i$. We will call the algorithms with adjustment \eqref{eq:adjust} adjusted algorithms. For example, the result of the $k$-th iteration of the Cadzow iterations method can be expressed as $\widehat\tsS_k = \calT^{-1}(\Pi_\calH \Pi_{\calM_r})^k \calT \tsX$. Then the result of the $k$-th iteration of the adjusted Cadzow iterations method is $\widehat\tsS_k^*=\calA(\widehat\tsS_k)$.
	\end{remark}

\section{Numerical examples}
\label{sec:simul}
Let us carry out numerical experiments for analysis of performance of the considered methods. Comparison of is methods was performed on two examples, with a sine-wave signal and an exponentially-modulated sine-wave signal.
Since the results of the method comparison are very similar, only results for the sine-wave signal are presented.

Let the signal $\tsS = (s_{1}, \ldots, s_N)$ of length $N = 40$ and rank $r=2$ have form:
\begin{equation*}
...
One can see that the method with a smaller limit error is the one with a slower convergence rate. For parameters involved to the simulations, the Cadzow($0.1$) method with the window length $L=8$ has the smallest limit error. At the same time, this set of parameters corresponds to both the slowest convergence and the most uniform weights.
...
\smallskip
Let us take Extended and Weighted Cadzow iterations into consideration and consider the spreading of the estimation errors along the series. The maximal number of iterations equal to 100 is taken for the stop criterion STOP1 (this choice yields the error close to the limiting value); the stop criterion STOP2 for inner iterations is as follows:
$\frac{\|\bfY_k - \bfY_{k+1}\|^2}{LK} < 10^{-4}$. Initial left and right extended values $\tsL_{L-1}$ и $\tsR_{L-1}$ in Extended Cadzow iterations are obtained using vector SSA forecasting \cite[chapter 2.3.1]{Golyandina.etal2001}.

Figures~\ref{fig:s1_it1}~and~\ref{fig:s1_it100} show the dependence of RMSE on numbers of the series points. Figure~\ref{fig:s1_it1} shows errors at the first iteration, Figure~\ref{fig:s1_it100} shows errors at the 100-th iteration. It is clearly seen that the Extended Cadzow method is the most precise in both cases. The Cadzow and Cadzow-$\widehat\bfC$ methods are the best at the first iteration among the set of methods without inner iterations. The best method in the limit (after 100-th iteration, results do not change significantly further) is the Cadzow($\alpha=0.1$) one; this is not surprising according to Figure~\ref{img_cadzowspeed2}.
...
Thus, the numerical examples confirm hypotheses and conclusions itemized in Section~\ref{sec:comments}.
...
Equal weights were achieved by using inner iterations, which converge to local minimum only and also make the algorithms with time-consuming. It appears that the use of methods without inner iterations leads to approximately equal weights only.
...
Let us consider modifications of SSA, which are produced by the first iteration of the considered oblique Cadzow iterative algorithms described in Section~\ref{sec:ObliqueCadzow}. Recall that the Cadzow($1$) iterative algorithm produces the conventional Basic SSA method \cite{Golyandina.etal2001,Golyandina.Zhigljavsky2012}, while the first iteration of a general oblique Cadzow algorithm can be considered as a
...
of the considered iterative algorithms. Notions of exact, approximate and asymptotical (as the series length tends to infinity) separability together with the asymptotic separability rate are introduced for
...
\end{equation}
where $X^{k,i}$ is the $i$-th row of matrix $\bfX^k$, $k = 1, 2$, and $(\cdot, \cdot)_\bfC$ is the oblique inner product in $\sfR^K$ generated by a matrix $\bfC$ defined as follows: $(X, Y)_\bfC = X \bfC Y^\sfT$ (here $X$ and $Y$ are row vectors), $\| \cdot \|_\bfC$ is the norm with respect to this inner product. We say that series $\tsX_1$ and $\tsX_2$ are \emph{weakly $\varepsilon$-separable} if
\begin{equation}\label{weak_sep_eq}
\rho = \max\Big(\max_{1 \le i,j \le K}|\rho^c_{i,j}|, \max_{1 \le i,j \le L}|\rho^r_{i,j}|\Big) < \varepsilon.
\end{equation}
We are interested in the order of $\varepsilon$ for different matrices $\bfC$, where the series $\tsX_k$, $k=1,2$, consist of the first $N$ terms of infinite series $\tsX_k^\infty$.
...
\smallskip
Let us consider the separability rate for Cadzow($\alpha$) iterations, introduced in Section~\ref{sec:cadzow_alpha}.

\begin{proposition}
	\label{prop:separ1} 
	Let $\tsX_1 = (\cos(2 \pi \omega k), k = 1, 2, \ldots)$, where $0<\omega <0.5$, be a sine wave, $\tsX_2 = (c, c, \ldots)$ be a constant series,  $L,K\ra \infty$ such that $h = h_L = N/L$, where $N=L+K-1$, be integer, and $\bfC=\bfC(\alpha)$ be defined in \eqref{zhigweights}, i.e.  $\bfC$ is diagonal matrix with diagonal elements:
	\begin{equation*}
	c_k = \begin{cases}
...
To prove the theorem, we should evaluate the order of the expressions:
	...
	Therefore, since the series $\tsX_2$ is not constant, the numerator in \eqref{th5_sep1} has order $O(1)$, while denominator has order $O(L)$. Thus, we obtain the order $1/L$.
