...
\begin{proposition}
	...
	\begin{enumerate}
		...
		\begin{equation*}
		C_{L,K} = C_{L(N),K(N)} = \sum_{\substack{1 \le k \le K: \\ c_k = 1}}\cos(2 \pi \omega k),
		\end{equation*} and
		\begin{equation*}
		D_{L,K} = D_{L(N),K(N)} = \sum_{\substack{1 \le k \le K: \\ c_k = 1}}\cos^2(2 \pi \omega k).
		\end{equation*}
		\item If $h$ is limited above by a constant, then $C_{L,K} = O(1)$, and $\rho$ has order $\max\left(\frac{1}{L}, \frac{1}{\alpha K}\right)$.
		\item If there exists small $\delta$, $0 < \delta < 1/2$, such that $2~L(N)~\omega~\in~\bigcup_{k \in \sfZ} [k + \delta, k + 1 - \delta]$ for every $N$, where $\sfZ$ is the set of integers, then $C_{L, K}$ has order $O(1)$, $D_{L, K}$ has order $\Theta(N/L)$, and $\rho$ has order $\max\left(\frac{1}{L}, \frac{1}{(1-\alpha)N/L+\alpha K}\right)$.
	\end{enumerate}
...
where
    \begin{gather*}
	C_{L, K} = \sum_{\substack{1 \le k \le K: \\ c_k = 1}}\cos(2 \pi \omega k),
	\\
...
and
    ...
    3. The condition $2 L(N) \omega \in \bigcup_{k \in \sfZ} [k + \delta, k + 1 - \delta]$ provides upper bound for $C_{L,K}$ and lower bound for $D_{L,K}$, since it limits $|\csc(\pi L(N) \omega)|$ in \eqref{sumcos} for $C_{L,K}$ and $|\csc(2 \pi L(N) \omega)|$ in \eqref{sumsqcos} for $D_{L,K}$.
\end{proof}
