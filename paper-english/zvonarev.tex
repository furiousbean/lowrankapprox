...
\begin{proposition}
	...
	\end{cases}
	\end{equation*}
	where $0 < \alpha(N) \le 1$. Then
	\begin{enumerate}
		\item $\rho$ given by \eqref{weak_sep_eq} has following order: $\rho~=~O\left(\max\left(\frac{1}{L}, \frac{(1-\alpha)C_{L,K}+\alpha}{(1-\alpha)D_{L,K}+\alpha K}\right)\right)$, where
		\begin{equation*}
		...
		\end{equation*}
		\item If $h$ is bounded by a constant, then $\rho~=~O(\max\left(\frac{1}{L}, \frac{1}{\alpha K}\right))$.
...
and
    ...
    2. $C_{L,K}$ is exactly the maximum of sums, each of $h$ cosines, therefore, the absolute value of $C_{L,K}$ is not larger than $h$, and, if $h$ is bounded by a constant, then $|C_{L,K}|$ is bounded by the same constant, so, $C_{L,K} = O(1)$.
    
    3. The condition $2~L(N)~\omega~\in~ \sfR \setminus \left(\bigcup_{k \in \sfZ} [k - \delta, k + \delta] \right)$ provides upper bound for $C_{L,K}$ and lower bound for $D_{L,K}$, since it limits $|\csc(\pi L(N) \omega)|$ in \eqref{sumcos} for $C_{L,K}$ and $|\csc(2 \pi L(N) \omega)|$ in \eqref{sumsqcos} for $D_{L,K}$, therefore, $C_{L, K}$ has order $O(1)$, $D_{L, K}$ has order $\Omega(N/L)$.
\end{proof}

\begin{remark}
	Here we use the following definitions for Big-O notation:
	\begin{gather*}
	f(n) \in O(g(n)) \Leftrightarrow \exists k>0 \exists n_0 \forall n > n_0: \, |f(n)| \le k |g(n)|, \\
	f(n) \in \Omega(g(n)) \Leftrightarrow \exists k>0 \exists n_0 \forall n > n_0: \, |f(n)| \ge k |g(n)|.
	\end{gather*}
\end{remark}

\begin{remark}
	Let us suppose that we choose $L(N)$ such that $\rho$ has order $\max\left(\frac{1}{L}, \frac{1}{(1-\alpha)N/L+\alpha K}\right)$. Then the optimal choice for $L$ is $L \approx \frac{\alpha(N + 1) + \sqrt{\alpha^2(N+1)^2 + 4N(1  - \alpha^2)}}{2(1 + \alpha)}$; the rate of separability remains its order $1/N$ (but with worse multiplier) for $\alpha(N) \to c > 0$, where $c$ is some constant. In the case of smaller $\alpha(N) = O(N^{-\beta})$ rate of separability becomes equal $O(N^{\beta - 1})$ for $0 \le \beta \le 0.5$, and $O(1/\sqrt{N})$ for $\beta > 0.5$.
\end{remark}
