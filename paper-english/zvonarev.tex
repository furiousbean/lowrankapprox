...
\end{multline*}
and the series $\tsX = \tsS + \tsU \odot \tsN$ be observed, where  $\tsN$ is Gaussian white noise with mean equal to $0$ and variance equal to $1$, $\odot$ is element-wise vector product, $\tsU = (u_{1}, \ldots, u_N)$, $u_k = 353.17 (0.9967)^k$. Note that the noise is actually modulated gaussian white noise. Accuracy of a signal estimate $\widehat\tsS$ is measured as root mean-square error (RMSE) using 1000 simulations. Only Cadzow($\alpha$) methods with various $\alpha$ were investigated, $L = 84$.  The following STOP1 criterion was taken: $\frac{\|\calT^{-1}(\tsY_k) - \calT^{-1}(\tsY_{k + 1})\|^2}{N} < 10^{-4}$. Consider Table~\ref{rltable} which shows RMSE for $\tilde \tsS$ as an estimate of $\tsS$ (i.e., signal estimation error), RMSE for $\tilde \tsS$ as an estimate of the initial series $\tsX$ (i.e., series approximation error), and distance to the actual $\tsX^*$ series.

\begin{table*}
	\caption{Comparison of methods by RMSE and distance to $\tsX^*$ on real-life example}\label{rltable}
	
	\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}ccccc}
		\hline
		$P$: & $\tsS$ & $\tsX$ & $\tsX^*$ \\
		\hline
		Cadzow, $\alpha = 1$ & 130.45 & 264.94 & 283.58 \\
		\hline
		Cadzow, $\alpha = 0.3$ & 127.25 & 263.25 & 280.94 \\
		\hline
		Cadzow, $\alpha = 0.1$ & 127.41 & 262.21 & 276.67 \\
		\hline
		Cadzow, $\alpha = 0.05$ & 130.77 & 262.71 & 274.00 \\
		\hline
	\end{tabular*}
\end{table*}

Since the noise model is not actually white, smaller $\alpha$ tends to overfitting of the series and worse signal estimation. 

As we can see, smaller $\alpha$ tends to better approximation of $\tsX^*$ and estimation of signal $\tsS$, but, according to our investigation, only $\alpha = 1$ and $\alpha = 0.3$ can be trusted. If $\alpha = 0.05$, the separability becomes worse, and small signal-to-noise ratio causes far away from global minimum convergence of algorithm.

Figure \ref{fig:rl} shows the best approximation of the actual series $\tsX^*$ obtained by Cadzow($0.3$) algorithm. Dotted line shows actual series, while solid line shows L-rank approximation obtained by the algorithm.

\begin{figure}[!hhh]
	%	\begin{center}
	\includegraphics[width = \columnwidth]{rlimage.pdf}
	\caption{`Fortified wine' series: application of Cadzow($0.3$) algorithm}
	\label{fig:rl}
	%	\end{center}
\end{figure}
