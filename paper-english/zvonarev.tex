...
%However, it will be shown in Appendix that the convergence in Algorithm \ref{sec:simul} is very slow for small $\alpha$.
...
Influence of parameter $\alpha$ in the class of Cadzow($\alpha$) iterations is investigated in Appendix~\ref{sec:app} on the example of separability of constant and sine-wave signals. It is shown that small values of $\alpha$ provide poor separability.
...
Note that we can compare numbers of iterations to achieve a given accuracy to compare the methods without inner iterations by computational complexity, since it was shown that the computational costs of one iteration are approximately the same.
...
This result can be found in \cite[Chapter 6.1]{Golyandina.etal2001}. %This result relates to the accuracy of first iteration of Cadzow iterations.
...
\begin{proposition}
	\label{prop:separ2}
	Let $\tsX_1 = (c, c, \ldots)$ be some a constant series and $\tsX_2 = (\cos(2 \pi \omega k), k = 1, 2, \ldots)$, where $0<\omega <0.5$, $L,K\ra \infty$ and $\bfC$ defined in Cadzow-$\widehat\bfC$ iterations.
