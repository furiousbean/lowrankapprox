...
\begin{table*}
	\caption{Comparison of methods by the errors of signal estimation and series approximation for the `Fortified wine' series and its simulated copies.}
\label{tab:rltable}
...
One can expect that the Cadzow($0.2$) algorithm provides one of the most accurate finite-rank estimations of the signal.
...
Equal weights were achieved by algorithms using inner iterations, which converge to a local minimum only and also make the algorithms very time-consuming. It appears that the use of methods without inner iterations (Cadzow-type methods) leads to approximately equal weights only.
