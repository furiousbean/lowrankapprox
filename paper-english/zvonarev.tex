...
\label{sec:appB}
Let us consider the series `Fortified wine' (fortified wine sales, Australia, monthly, from January 1980 till June 1994). These series have the following structure: a smooth exponential trend, an annual seasonality of a complex form, and a noise. We truncate the series by the first 168 steps, build the parametric form using SSA framework with ESPRIT, estimate the RMSE for various Cadzow($\alpha$) the same way as in Section \ref{sec:simul}, and apply methods to the actual series.
...
\begin{multline*}
s_{k} = 3982.46 (0.9967)^k + \\ 
1221.36 (0.9934)^k \sin(\frac{\pi k}{6} - 2.246) + \\
418.38 (1.0002)^k \sin(\frac{\pi k}{2} + 2.333) + \\
245.05 (1.0024)^k \sin(\frac{\pi k}{3} + 1.659) + \\ 
193.18 (0.9987)^k \sin(\frac{5 \pi k}{6} + 1.476) + \\
397.77 (0.9854)^k \sin(\frac{2 \pi k}{3} - 2.862)
\end{multline*}
and the series $\tsX = \tsS + \tsU \odot \tsN$ be observed, where  $\tsN$ is Gaussian white noise with mean equal to $0$ and variance equal to $1$, $\odot$ is element-wise vector product, $\tsU = (u_{1}, \ldots, u_N)$, $u_k = 253.08 (0.9967)^k$. Note that the noise is actually modulated gaussian white noise. Accuracy of a signal estimate $\widehat\tsS$ is measured as root mean-square error (RMSE) using 1000 simulations. Only Cadzow($\alpha$) methods with various $\alpha$ were investigated, $L = 84$.  The following STOP1 criterion was taken: $\frac{\|\calT^{-1}(\tsY_k) - \calT^{-1}(\tsY_{k + 1})\|^2}{N} < 10^{-4}$. Consider Table~\ref{rltable} which shows RMSE for $\tilde \tsS$ as an estimate of $\tsS$ (i.e., signal estimation error) and RMSE for $\tilde \tsS$ as an estimate of the initial series $\tsX$ (i.e., series approximation error).

\begin{table*}
	\caption{Comparison of methods by RMSE on real-life example}\label{rltable}
	
	\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}ccccc}
		\hline
		$P$: & $\tsS$ & $\tsX$  \\
		\hline
		Cadzow, $\alpha = 1$ & 88.342 & 188.865 \\
		\hline
		Cadzow, $\alpha = 0.3$ & 86.255 & 187.678 \\
		\hline
		Cadzow, $\alpha = 0.1$ & 84.638 & 186.121  \\
		\hline
		Cadzow, $\alpha = 0.05$ & 85.842 & 185.877 \\
		\hline
	\end{tabular*}
\end{table*}

Since the noise model is not actually white, small $\alpha$ tends to overfitting of the series and worse signal estimation.

Let us consider Figure \ref{fig:rl} showing application of these algorithms to the actual series $\tsX^*$. Dotted line shows actual series, while solid line shows L-rank approximation obtained by algorithms. As we can see, smaller $\alpha$ tends to better approximation of $\tsX^*$ and estimation of signal $\tsS$, but, according to our investigation, only $\alpha = 1$, $\alpha = 0.3$ and $\alpha = 0.1$ can be trusted.

\begin{figure*}
	%	\begin{center}
	\includegraphics[width = \textwidth]{rlimage.pdf}
	\caption{`Fortified wine' series: application of Cadzow($\alpha$) algorithms}
	\label{fig:rl}
	%	\end{center}
\end{figure*}
