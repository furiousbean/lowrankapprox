...
Consider a problem of projecting a point $x$ to the set $\calH \cap \calM$ in Hilbert space $\sfX$ with inner product $\langle \cdot, \cdot \rangle$, where $\calH$ is some closed linear subspace, $\calM$ is subset closed in topology, and also closed with respect to scalar multiplication, i.e.
if $z \in \calM$, then $\alpha z\in \calM$ for any $\alpha$. This fact is crucial for the proof of the following theorem. Note that  $\calM$ is neither a linear, nor convex set.
...
\ee
$\|\cdot\|$ is the corresponding norm.

To present the algorithm's scheme for this problem, let us introduce the projectors to the subset $\calM$ and subspace $\calH$ with respect to a norm $\|\cdot\|$: $\Pi_{\calM}$ is the projector to $\calM$,
$\Pi_{\calH}$ is the projector to $\calH$.
Note that the projection to $\calM$ can be non-unique; further we suppose that in the case of ambiguity any closest point is chosen. The projector to $\calH$ is evidently orthogonal, while $\Pi_{\calM}$ is orthogonal due to the following proposition. 

\begin{proposition} \label{pythaprop}
	Let $\sfX$ be Hilbert space, $\calM \subset \sfX$ be a subset closed with respect to scalar multiplication, $\Pi_\calM$ be the projection operator to $\calM$. Then for any $x \in \sfX$ the following equation (``Pythagorean theorem'') is true: $\|x\|^2~=~\|x~-~\Pi_\calM x\|^2~+~\|\Pi_\calM x\|^2$.
\end{proposition}
%\begin{proof}
...
\begin{theorem}
	\label{th:converg}
	\begin{enumerate}
		Let the set $\calM$ be closed. Then
		\item $\|y_k - \Pi_{\calM}y_k\| \to 0$ as $k \to +\infty$, $\|\Pi_{\calM}y_k - y_{k+1}\| \to 0$ as $k \to +\infty$.
		\item Let $\calM \cap B_1$ be compact set, where $B_1=\{z: \|z\|~\le~1\}$ is closed unit ball. Then there exists a convergent subsequence of points $y_{i_1}, y_{i_2}, \ldots$ such that its limit $y^*$ belongs to $\calM \cap \calH$.
	\end{enumerate}
\end{theorem}
\begin{proof}
	Let us use following inequalities:
	\begin{multline}
	\label{chuprop}
	\|y_k - \Pi_{\calM} y_k\| \ge \|\Pi_{\calM} y_k - y_{k + 1}\| \ge \\ \|y_{k+1} - \Pi_{\calM} y_{k + 1}\|.
	\end{multline}
	This is the direct consequence from the fact that $\Pi_\calM y_{k+1}$ is not farther from $y_{k+1}$ than any other point from $\calM$ including $\Pi_\calM y_k$. Similar conclusion can be obtained with $\Pi_\calH$.
	\begin{enumerate}
		\item According to inequalities \eqref{chuprop}, the sequences $\|y_k~-~\Pi_{\calM} y_k\|$, $k = 1, 2, \ldots$, and $\|\Pi_{\calM} y_k - y_{k + 1}\|$, $k = 1, 2, \ldots$, are non-increasing. It is obvious that they are limited below by zero. Therefore, they have the same limit $c$ due to \eqref{chuprop}.
		
		Let us prove that $c = 0$ assuming the opposite $c > 0$. Then there exists $d > 0$ such that $\|y_k - \Pi_{\calM} y_k\| > d$ and $\|\Pi_{\calM} y_k - y_{k + 1}\| > d$ for any $k = 1, 2, \ldots$. In accordance to Proposition~\ref{pythaprop}, the following equality is valid: $\|y_k \|^2~=~\|y_k~-~\Pi_{\calM} y_k\|^2~+~\|\Pi_{\calM} y_k \|^2$. Since the space $\calH$ is linear, the following equality is valid too:
		...
		\item Consider the sequence $(\Pi_{\calM} y_k,\, k = 1, 2, \ldots)$. It is bounded because $\|\Pi_{\calM} z\| \le \|z\|$ and $\|\Pi_{\calH} z\| \le \|z\|$ for any $z \in \sfX$ (by Proposition \ref{pythaprop}). Then a convergent subsequence $(\Pi_{\calM} y_{i_k})$ can be chosen; denote by $y^*\in\calM$ its limit, wherein $\|\Pi_{\calM} y_{i_k} - y_{i_k + 1}\| = \|\Pi_{\calM} y_{i_k} - \Pi_\calH \Pi_{\calM} y_{i_k}\| \to 0$ as $k \to + \infty$. Taking into consideration that $\|z - \Pi_\calH z\|$ is a composition of continuous mappings, we obtain that $\|y^* - \Pi_\calH y^*\| = 0$, $y^* \in \calM \cap \calH$. Finally, $\Pi_\calH$ is continuous mapping and therefore the sequence $(\Pi_\calH \Pi_{\calM} y_{i_k})$ converges to $y^*$. Thus, $y_{i_k + 1}$ is a required subsequence.
	\end{enumerate}
\end{proof}

Actually Proposition \ref{pythaprop} was in fact proved in \cite{Gillard.Zhigljavsky2013} for a particular case, furthermore inequalities (4.1) from \cite{Chu.etal2003} is the particular case of inequalities \eqref{chuprop}.

Now we substitute needed sets into common abstract scheme: let us take $X = \spaceR^{L\times K}$, i.e. the space of matrices of size $L \times K$ with some inner product, $\calH \subset \spaceR^{L\times K}$ is the space of Hankel matrices, $\calM = \calM_r\subset \spaceR^{L\times K}$ is the set of matrices of rank not larger than $r$. Iterative step \ref{eq:iter} for method of alternating projections is as follows:
\begin{equation*}
...
It is well known that the set $\calM_r$ is closed with respect to the conventional Frobenius norm. It can be proved that $\calM_r$ is closed with respect to any weighted norm \eqref{eq:w_inner_prod} when $m_{l,k} > 0$ and therefore the conclusion of Theorem~\ref{th:converg} holds in the case of positive weights.
\footnote{To improve the sentences about closeness}
Unit ball is obviously compact set in finite dimensional Euclidean space.
