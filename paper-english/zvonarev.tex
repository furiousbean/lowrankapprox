...
\end{cases},
\end{equation}
and $\bfY$ is the corresponding trajectory matrix for the series $\tsY$.
...
Note that the projection to $\calM_r$ can not be defined uniquely, but further we suppose that in the case of ambiguity any closest value is chosen. The projector to $\calH$ is evidently orthogonal, while $\Pi_{\calM_r}$ is orthogonal due to the following proposition.

\begin{proposition} \label{pythaprop}
	Let $\calX$ be Hilbert space, $\calM \subset \calX$ be a subset closed with respect to scalar multiplication, $\Pi_\calM$ be the projection operator to $\calM$. Then for any $x \in \calX$ the following equation (``Pythagorean theorem'') is true: $\|x\|^2~=~\|x~-~\Pi_\calM x\|^2~+~\|\Pi_\calM x\|^2$.
\end{proposition}
%\begin{proof}
...
\begin{proof}
	Let us define $y = \Pi_\calM x$, then:
	\begin{gather*}
	\|x\|^2 = \|x - y + y\|^2 = \\ \langle x - y, x - y \rangle + 2 \langle x - y, y \rangle + \langle y, y \rangle = \\ \|x - y \|^2 + \|y \|^2 + 2 \langle x - y, y \rangle.
	\end{gather*}
	From Proposition's statement it follows that $\langle x - y, y \rangle = 0$. Assume the contrary: $\langle x - y, y \rangle \ne 0$. Then there exists $\alpha$ such that $\langle x - \alpha y, \alpha y \rangle = \alpha \langle x - \alpha y, y \rangle = 0$: 
	\begin{equation*}
	\alpha = \frac{\langle x, y \rangle}{\langle y, y \rangle}.
	\end{equation*}
	$\alpha y$ lies in $\calM$ according to the property of $\calM$. It becomes clear that $\alpha y$ is closer to $x$ than $y$:
	\begin{gather*}
	\|x - y\|^2 - \|x - \alpha y\|^2 = \langle y, y \rangle - 2 \langle x, y \rangle + \frac{\langle x, y \rangle ^ 2}{\langle y, y \rangle} = \\ \frac{\langle x - y, y \rangle^2}{\langle y, y \rangle} > 0.
	\end{gather*}
	Hence, the contradiction with the fact that $y = \Pi_\calM x$ is the closest point to $x$ is acquired.
\end{proof}
...
\begin{theorem}
	\label{th:converg}
	\begin{enumerate}
		Let the set $\calM_r$ be closed in the topology generated by norm $\|\cdot\|$. Then
...
\begin{proof}
	...
	\begin{enumerate}
		...
		Let us prove that $c = 0$ assuming the opposite $c > 0$. Then there exists $d > 0$ such that $\|\bfY_k - \Pi_{\calM_r} \bfY_k\| > d$ and $\|\Pi_{\calM_r} \bfY_k - \bfY_{k + 1}\| > d$ for any $k = 1, 2, \ldots$. In accordance to Proposition~\ref{pythaprop}, the following equality is valid: $\|\bfY_k \|^2~=~\|\bfY_k~-~\Pi_{\calM_r} \bfY_k\|^2~+~\|\Pi_{\calM_r} \bfY_k \|^2$. Since the space $\calH$ is linear, the following equality is valid too:
		$\|\Pi_{\calM_r} \bfY_k \|^2~=\|\Pi_{\calM_r} \bfY_k~-~\Pi_\calH \Pi_{\calM_r} \bfY_k|^2~+~\|\Pi_\calH \Pi_{\calM_r} \bfY_k \|^2 = \|\Pi_{\calM_r} \bfY_k~-~\bfY_{k+1}|^2~+~\|\bfY_{k+1} \|^2$. Therefore,
		\begin{multline*}
...
\begin{figure}[!hhh]
	...
	\caption{RMSE and average numer of iterations depending on $\alpha$.}
	\label{img_2axis}
	%	\end{center}
\end{figure}
