...
To present the algorithm's scheme for this problem, let us introduce the projectors to the subspaces $\calM_r$ and $\calH$ with respect to a norm $\|\cdot\|$: $\Pi_{\calM_r}$ is the projector to $\calM_r$,
$\Pi_{\calH}$ is the projector to $\calH$.
Note that the projection to $\calM_r$ can be non-unique; further we suppose that in the case of ambiguity any closest matrix is chosen. The projector to $\calH$ is evidently orthogonal, while $\Pi_{\calM_r}$ is orthogonal due to the following proposition, which was in fact proved in \cite{Gillard.Zhigljavsky2013} for a particular case.
...
\label{sec:appB}
Let us consider the series `Fortified wine' (fortified wine sales, Australia, monthly, from January 1980 till June 1994).
