...
%\setcounter{page}{1}
    ...
    % Тема
    \topic{\normalfont\scshape %
    	Итеративные алгоритмы взвешенной аппроксимации временными рядами конечного ранга}
...
\section{Общая схема}
Рассмотрим задачу проектирования точки $x$ на множество $\calH~ \cap~\calM$ в гильбертовом пространстве $\sfX$ со скалярным произведением $\langle \cdot, \cdot \rangle$, где $\calH$ и $\calM$ замкнуты в порожденной скалярным произведением топологии, $\calH$ --- линейное подпространство, а также $\calM$ замкнуто относительно операции умножения на скаляр, то есть, если $z \in \calM$, то $\alpha z\in \calM$ для любого вещественного $\alpha$. Это ключевой факт для доказательства следующей теоремы. Заметим, что $\calM$ не является ни линейным, ни выпуклым множеством.
...
Алгоритм был применен к 1000 реализаций модельного ряда, и к ряду `Fortified wine'.
...
\begin{proposition}
	\label{prop:separ1}
	Пусть $\tsX_1^\infty = (\cos(2 \pi \omega k), k = 1, 2, \ldots)$, где $0<\omega <0.5$ --- гармонический ряд, $\tsX_2^\infty = (c, c, \ldots)$ --- константный ряд,  $L(N),K(N)\ra \infty$, где $N=L+K-1$, $h = h_N = \lfloor N/L \rfloor$, и $\bfC=\bfC(\alpha(N))$, определенная в \eqref{zhigweights}, т.е. $\bfC$ диагональная матрица с диагональными элементами:
	\begin{equation*}
	c_k = \begin{cases}
	...
	\begin{enumerate}
		...
		\end{equation*}
		\item Если $h$ ограничено константой, то $\rho~=~O(\max\left(\frac{1}{L}, \frac{1}{\alpha K}\right))$.
...
\begin{proof}
	...
	2. $C_{L,K}$ это в точности максимум из сумм, каждая из которых состоит из $h$ косинусов, таким образом, абсолютное значение $C_{L,K}$ не превосходит $h$, и, если $h$ ограничено константой, тогда $|C_{L,K}|$ ограничено той же константой, следовательно $C_{L,K} = O(1)$.
	
	3. Условие $2~L(N)~\omega~\in~ \sfR \setminus \left(\bigcup_{k \in \sfZ} [k - \delta, k + \delta] \right)$ дает верхнюю оценку для $C_{L,K}$ и нижнюю оценку для $D_{L,K}$, так как оно ограничивает $|\csc(\pi L(N) \omega)|$ в \eqref{sumcos} для $C_{L,K}$ и $|\csc(2 \pi L(N) \omega)|$ в \eqref{sumsqcos} для $D_{L,K}$, следовательно, $C_{L, K}$ имеет порядок $O(1)$, $D_{L, K}$ имеет порядок $\Omega(N/L)$.
\end{proof}

Предположим, что $L(N)$ выбирается так, что $\rho$ имеет порядок $\max\left(\frac{1}{L}, \frac{1}{(1-\alpha)N/L+\alpha K}\right)$. Тогда оптимальный выбор для $L$ это $L \approx \frac{\alpha(N + 1) + \sqrt{\alpha^2(N+1)^2 + 4N(1  - \alpha^2)}}{2(1 + \alpha)}$; порядок разделимости $1/N$ сохраняется для зафиксированного параметра $\alpha$, но в случае меньших $\alpha(N) = O(N^{-\beta})$ порядок разделимости становится равным $O(N^{\beta - 1})$ для $0 \le \beta \le 0.5$, и $O(1/\sqrt{N})$ для $\beta > 0.5$.
%
...
\section{Приложение 2: Быстрая реализация внутренних итераций алгоритма Extended Cadzow}
