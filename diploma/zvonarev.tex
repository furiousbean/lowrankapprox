...
\begin{proof}
	...
	Так как ряд $\tsX_1$ не является константным, числитель в \eqref{th5_sep1} имеет порядок $O(1)$, в то время как знаменатель имеет порядок $\Theta(L)$. Следовательно, получаем порядок $1/L$.
