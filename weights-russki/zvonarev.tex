...
Рассмотрим выражение $\varphi(C)/2$, где $\varphi(C)$ определено в \eqref{eq:commonw}. Преобразуем его, избавимся от константного слагаемого, а множество $\calC$ из \eqref{eq:commonw_cond} запишем в виде набора линейных ограничений. Получим следующую эквивалентную формулировку:
...
\begin{enumerate}
	...
	Вторая стратегия используется при $j = 1$ и основана на следующем факте: решения задач при примерно одинаковом отношении $N$ к $L$ и одинаковом $\alpha$ схожи. Например, на рисунке \ref{img:scale} изображены два отнормированных (умноженных на $N$) решения задачи \ref{problem:quadtf} при $N = 200$, $L = 60$, $\alpha = 0.1$ и $N = 1000$, $L = 300$, $\alpha = 0.1$ соответственно.
	...
	\begin{algorithm}
		...
		\begin{enumerate}
			...
			\item Определить функцию $s(i) = [\frac{(i - 1)(K_\gamma - 1)}{K - 1}~+~1]$ перехода к ``уменьшенным'' координатам.
			...
			\begin{enumerate}
				...
				\item Проверить на точке $C^\star$ все условия вида \eqref{eq:positw_min} и \eqref{eq:positw_max}. Если все они выполняются, то ответ --- $X_0 = C^\star$, $\sfW_0 = \sfW$ и STOP, иначе добавить к рабочему множеству $W$ индекс любого ограничения, которое не выполняется.
			\end{enumerate}
		\end{enumerate}
	\end{algorithm}
